...
Deno.test("week and slot contract is deterministic", () => {
  const pragueSunday = new Date("2026-08-16T21:59:59Z");
  const pragueMonday = new Date("2026-08-16T22:00:00Z");
  assertEquals(mondayPrague(pragueSunday), "2026-08-10");
  assertEquals(mondayPrague(pragueMonday), "2026-08-17");
  assert(isCurrentWeek("2026-08-10", pragueSunday));
  assert(isCurrentWeek("2026-08-17", pragueMonday));
...
Deno.test("PNG is rejected because supported clients upload normalized JPEG", () => {
  assertEquals(validateAndSanitizeImage(png(true), "image/png"), null);
});
