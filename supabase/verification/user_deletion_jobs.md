...
Step 3 includes applying `20260811212458_create_admin_security_audit.sql`, and
it is a hard prerequisite for step 5: `admin-manage-user` writes an audit
`attempt` row before every mutation and fails closed without that table.
Deploying it first makes deletion — and every other admin action — return HTTP
`500` `{"error":"Unable to record the account action"}` with
`Unable to persist admin security audit` in the function logs. See
`admin_mfa_enforcement.md`, "Hard ordering requirement", for the full symptom
and recovery.
