...
export function LoadingBlock({ label = 'Loading activity…' }: { label?: string }) {
  return <div className="rounded-2xl border border-outline bg-surface-elevated p-8 text-sm text-text-secondary">{label}</div>
}

export function ErrorBlock({ message = 'Activity could not be loaded. Refresh the page to retry.' }: { message?: string }) {
  return <div role="alert" className="rounded-2xl border border-error/40 bg-error/10 p-5 text-sm text-error">{message}</div>
}
...
export function ExerciseVisual({ exercise, name, className = '' }: {
...
export function WorkoutCard({ workout, compact = false }: { workout: WorkoutRow; compact?: boolean }) {
  ...
  return (
    <Link
      to={`/activity/workouts/${workout.id}`}
      className="group flex h-full flex-col rounded-2xl border border-outline bg-surface-elevated p-5 text-inherit no-underline transition-colors hover:bg-surface-highest"
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold text-text-accent">
            {workout.day_of_week == null ? 'Flexible day' : DAY_NAMES[workout.day_of_week]}
          </p>
...
export function StartButton({ pending, onClick, label = 'Start workout' }: { pending: boolean; onClick: () => void; label?: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={pending}
      className="inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-xl border-0 bg-action-primary px-5 text-sm font-bold text-on-action-primary transition-opacity hover:opacity-85 disabled:cursor-wait disabled:opacity-50"
    >
      <Play size={17} fill="currentColor" /> {pending ? 'Opening…' : label}
    </button>
  )
}
