...
export default function ActivityHub() {
  ...
  // The daily quote and the page header are driven by their own query (or by
  // nothing at all), so they stay mounted while the four workout queries
  // resolve — only the region below swaps between loading, error and content.
  return (
    <ActivityPage>
      <DailyQuoteCard />

      <PageIntro eyebrow="Tréning" title="Aktivita" description="Tvoj plán, sledovanie tréningu, knižnica cvikov a pokrok na jednom mieste." action={<Link to="/activity/workouts/new" className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-outline bg-surface px-4 text-sm font-semibold text-text-primary no-underline hover:bg-surface-elevated"><Plus size={16} /> Vlastný tréning</Link>} />
                      ...
                      .map(({ workout, log }) => (
                        <div key={workout.id} className={`flex items-center gap-3 rounded-2xl border p-4 ${log ? 'border-success/40 bg-success/5' : 'border-outline bg-surface-elevated'}`}>
                          ...
                          {log ? (
                            ...
                            <button type="button" disabled={startMutation.isPending} onClick={() => startMutation.mutate(workout)} className="min-h-11 flex-shrink-0 cursor-pointer rounded-xl border-0 bg-action-primary px-4 text-xs font-bold text-on-action-primary disabled:opacity-40">
                              Začať
                            </button>
                          )}
                        </div>
            ...
            <section className="space-y-3">
              ...
              {plan.length ? (
                ...
                <div className="rounded-2xl border border-dashed border-outline p-6 text-sm text-text-secondary">
                  ...
                  <Link to="/messages" className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-action-primary px-4 text-sm font-bold text-on-action-primary no-underline">
                    Napísať trénerke
                  </Link>
                </div>
              )}
