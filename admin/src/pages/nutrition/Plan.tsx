...
export default function Plan({ embedded = false }: { embedded?: boolean }) {
  ...
  if (!plan)
    return (
      <EmptyState
        title="Žiadny plán"
        message="Tréner ti zatiaľ nepriradil jedálniček."
        action={
          <Link to="/messages" className="inline-flex min-h-10 items-center rounded-xl bg-action-primary px-4 text-sm font-bold text-on-action-primary no-underline">
            Napísať trénerke
          </Link>
        }
      />
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex gap-2 overflow-x-auto rounded-2xl border border-outline-subtle bg-surface-elevated p-2">
        {DAYS.map((d, i) => (
          <Chip key={d} selected={i === day} onClick={() => setDay(i)}>
            {d}
          </Chip>
        ))}
      </div>
      ...
      <div className="grid items-start gap-4 lg:grid-cols-2">
        {meals.map(meal => {
          ...
          const recipeIdBySnapshotName = new Map(meal.meal_plan_recipes.filter(recipe => recipe.recipe_id && recipe.snapshot_recipe_name).map(recipe => [recipe.snapshot_recipe_name!, recipe.recipe_id!]))
          const multiplierBySnapshotName = new Map(meal.meal_plan_recipes.filter(recipe => recipe.snapshot_recipe_name && recipe.portion_multiplier !== 1).map(recipe => [recipe.snapshot_recipe_name!, recipe.portion_multiplier]))
          return (
                ...
                {meal.meal_foods.map(f => {
                  ...
                  const content = (
                    <>
                      <span className="flex min-w-0 items-center gap-1 font-medium text-text-primary">
                        ...
                        {multiplierBySnapshotName.has(f.name) && <span className="flex-shrink-0 rounded-full bg-accent/10 px-1.5 py-0.5 text-[10px] font-bold text-accent">{multiplierBySnapshotName.get(f.name)}× porcia</span>}
                        {recipeId && <ChevronRight size={15} className="flex-shrink-0 text-accent" aria-hidden="true" />}
                      </span>
                      <span className="flex-shrink-0 text-text-secondary">
                        {Math.round(f.amount_grams)} g · {Math.round(f.calories)} kcal
                      </span>
                    </>
                  )
                  ...
                  return (
                    <li key={f.id} className="text-sm">
                      {recipeId ? (
                        <Link to={`/nutrition/recipes/${recipeId}`} aria-label={`Otvoriť recept ${f.name}`} className="flex min-h-11 items-center justify-between gap-4 rounded-lg py-2.5 outline-none transition-colors hover:text-accent focus-visible:ring-2 focus-visible:ring-accent">
                          {content}
                        </Link>
                      ) : (
