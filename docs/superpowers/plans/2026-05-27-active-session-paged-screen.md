# Active Session Screen — Paged Exercise Design Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the scrollable list of all exercises in `ActiveSessionScreen` with a `HorizontalPager` that shows one exercise at a time, with a dot indicator, swipe + button navigation, and an overflow menu.

**Architecture:** `HorizontalPager` from `compose.foundation` drives paging; `PagerState.currentPage` is the single source of truth for which exercise is active, the dot indicator, and which nav buttons are visible. `ExerciseDraft` gains `muscleGroup`, `tips`, and ensures `videoUrl` is populated from `toDraft()`.

**Tech Stack:** Compose Multiplatform 1.10.2, `androidx.compose.foundation.pager.HorizontalPager`, Material3, Koin, KMP (commonMain)
...
| File | Change |
|------|--------|
| `composeApp/src/commonMain/kotlin/com/coachfoska/app/presentation/workout/SessionDraft.kt` | Add `muscleGroup`, `tips` to `ExerciseDraft`; populate all three new fields in `toDraft()` |
| `composeApp/src/commonMain/kotlin/com/coachfoska/app/ui/workout/ActiveSessionScreen.kt` | Full redesign: `HorizontalPager`, `ExercisePage`, `ExerciseDotIndicator`, overflow menu, nav row |

No ViewModel, intent, or state changes required.

---

## Task 1: Extend ExerciseDraft

**Files:**
- Modify: `composeApp/src/commonMain/kotlin/com/coachfoska/app/presentation/workout/SessionDraft.kt`

- [ ] **Step 1: Add `muscleGroup` and `tips` fields to `ExerciseDraft`**

Replace the existing `ExerciseDraft` data class with:
...
- [ ] **Step 2: Update `WorkoutExercise.toDraft()` to copy all three fields**

Replace the existing `toDraft()` extension:
...
Expected: all tests pass (the new fields have defaults so no existing test breaks).

- [ ] **Step 5: Commit**

```bash
git add composeApp/src/commonMain/kotlin/com/coachfoska/app/presentation/workout/SessionDraft.kt
git commit -m "feat(workout): add muscleGroup, tips, videoUrl to ExerciseDraft"
```

---

## Task 2: Rewrite ActiveSessionScreen
...
The entire file below replaces the current `ActiveSessionScreen.kt`. Key structural changes:
- `ActiveSessionScreen` now hosts a `HorizontalPager` instead of a `LazyColumn`
- Three new private composables: `ExercisePage`, `ExerciseDotIndicator`
- Overflow menu in `CoachTopBar`'s `actions` slot
- Notes field moved into `ExercisePage` when `isLastExercise = true`
...
Expected: `BUILD SUCCESSFUL`. If there are import errors for `HorizontalPager` or `rememberPagerState`, confirm `compose.foundation` is in the module's `build.gradle.kts` dependencies (it already is). If `DropdownMenu`, `SuggestionChip`, or `HorizontalDivider` are unresolved, add the missing `material3` imports — they are all part of `androidx.compose.material3`.
...
Expected: all tests pass. No ViewModel or domain logic was changed.

- [ ] **Step 4: Commit**

```bash
git add composeApp/src/commonMain/kotlin/com/coachfoska/app/ui/workout/ActiveSessionScreen.kt
git commit -m "feat(workout): replace scrollable list with HorizontalPager in active session screen"
```
...
**Spec coverage:**
- ✅ HorizontalPager with swipe + buttons — Task 2
- ✅ Animated image, muscle group chip, tips, target sets × reps — Task 2 (`ExercisePage`)
- ✅ Dots with completion color and current highlighted by size — Task 2 (`ExerciseDotIndicator`)
- ✅ Finish button only on last page — Task 2 (nav row)
- ✅ Three-dots menu with Skip / Finish workout — Task 2 (overflow menu in `CoachTopBar`)
- ✅ Notes on last exercise page — Task 2 (`isLastExercise` guard)
- ✅ `muscleGroup` and `tips` in `ExerciseDraft` — Task 1

**Placeholder scan:** No TBDs, no "similar to Task N", all code is complete.

**Type consistency:**
- `ExerciseDraft.muscleGroup: String?` defined in Task 1, used in Task 2 ✅
- `ExerciseDraft.tips: String?` defined in Task 1, used in Task 2 ✅
- `ExerciseDraft.videoUrl: String?` already existed, now populated in Task 1, used in Task 2 ✅
- `ExerciseDotIndicator(exercises, currentPage, modifier)` defined and called correctly ✅
- `ExercisePage(exerciseIndex, draft, isLastExercise, notes, onNotesChange, onIntent)` defined and called correctly ✅
